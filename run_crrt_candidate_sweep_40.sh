...
sys.exit(0 if seed in set(df["seed"].astype(int)) else 1)
PY
}
...
run_scene3 () {
  ...
  for kc in "${CANDIDATES[@]}"; do
    OUT=~/ws_3dof/results/crrt_candidate_sweep_40/crrt_scene3_kc${kc}_40seed.csv

    for seed in $SEEDS; do
      if seed_done "$OUT" "$seed"; then
        echo "Classical Scene 3 | kc=${kc} | seed=${seed} already complete, skipping"
        continue
      fi

      echo "Classical Scene 3 | kc=${kc} | seed=${seed}"
...
run_scene4 () {
  ...
  for kc in "${CANDIDATES[@]}"; do
    OUT=~/ws_3dof/results/crrt_candidate_sweep_40/crrt_scene4_kc${kc}_40seed.csv

    for seed in $SEEDS; do
      if seed_done "$OUT" "$seed"; then
        echo "Classical Scene 4 | kc=${kc} | seed=${seed} already complete, skipping"
        continue
      fi

      echo "Classical Scene 4 | kc=${kc} | seed=${seed}"
