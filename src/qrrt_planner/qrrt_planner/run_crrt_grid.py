...
Summary:
    Command-line runner for Classical RRT on the discretized joint-space grid.
    ...
    It:
        ...
        4. Runs the classical grid-RRT planner.
    ...
    The actual planning logic lives in crrt_grid.py. This file mainly handles
    experiment setup, timing, reporting, and CSV output.
"""
...
from qrrt_planner.crrt_grid import crrt_grid
...
def main():
    """
    Parse arguments, run classical grid-RRT, and report/save metrics.
    """
    ...
    # Time only the planner run, including MoveIt validity calls made during planning.
    t0 = time.perf_counter()
    path_idx, stats = crrt_grid(
        q_start=q_start,
        q_goal=q_goal,
        bins=bins,
    ...
    print(f"classical calls:           {stats['classical_calls']}")
    print(f"classical good extensions: {stats['classical_successful_extensions']}")
    ...
    if args.csv_out:
        row = {
            ...
            "classical_calls": int(stats["classical_calls"]),
            "classical_successful_extensions": int(stats["classical_successful_extensions"]),
